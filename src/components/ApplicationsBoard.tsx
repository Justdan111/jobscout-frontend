...
export default function ApplicationsBoard() {
  ...
  const move = async (a: Application, stage: AppStage) => {
    setApps((prev) => prev.map((x) => (x.jobId === a.jobId ? { ...x, stage } : x)));
    await api.updateApplication(a.jobId, { stage });
  };
            ...
            <section key={s.key}>
              ...
              <div className="space-y-2">
                {(byStage[s.key] || []).map((a) => {
                  ...
                  return (
                    <div key={a.jobId} className="rounded-lg border border-line bg-surface p-3">
